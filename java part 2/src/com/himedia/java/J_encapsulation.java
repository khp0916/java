...
// * 캡슐화(Encapsulation)
// 객체 지향 프로그램의 기본 원칙 중 하나로, 데이터를 숨기고 그 데이터를 조작하는 메서드만 외부에 노출하여
...
// 이렇게 하면 객체 내부의 상태가 무분별하게 변경되는0 것을 막을 수 있다.
// 2. 접근 제어자
// 접근 제어자를 사용하여 클래스 멤버(필드와 메서드)에 대한 접근 권한을 제어한다.
// 이를 통해 어떤 클래스가 특정 멤버에 접근할 수 있는지를 정의한다.
// 3. Getter와 Setter
// 필드가 private로 설정된 경우, 외부에서 이 필드에 접근하기 위해 getter와 setter 메서드를 제공한다.
