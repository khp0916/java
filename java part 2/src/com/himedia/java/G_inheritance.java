...
// 이때, 부모 클래스의 메서드와 동일한 이름, 반환 타입, 매개변수를 가져야 한다.
...
// * 주요 특징
// 1. 메서드 재정의
// 오버라이딩은 부모 클래스에서 정의된 메서드를 자식 클래스에서 동일한 이름, 반환 타입, 매개변수를 사용해 다시 정의하는 것이다.
// 2. 상속 관계
// 오버라이딩은 상속 관계에 있는 클래스들 간에서만 가능하다.
// 자식 클래스가 부모 클래스의 메서드를 상속받아 이를 재정의할 때 오버라이딩이 발생한다.
// 3. 메서드 시그니처
// 오버라이딩하는 메서드는 부모 클래스에서의 메서드와 동일한 시그니처(이름, 매개변수, 반환타입)을 가져야한다.
// 4. @override 어노테이션
// 자바에서는 오버라이딩된 메서드 위에 어노테이션을 붙여 명시적으로 오버라이딩임을 표시할 수 있다.
