...
// * 프로세스와 스레드
// 프로세스란 '실행중인 프로그램'이다.
// 프로그램을 실행하면 OS 로부터 실행에 필요한 자원(메모리)을 할당받아 프로세스가 된다.
...
// 프로그래밍해야 한다.
// 만일 싱글 스레드로 서버 프로그램을 작성한다면 사용자의 요청 마다 새로운 프로세스를 생성해야하는데,
...
// 이 인스턴스를 Thread 클래스의 생성자의 매개변수로 제공해야 한다.

// -> 어느 쪽을 선택해도 별 차이는 없지만, Thread 클래스를 상속받으면 다른 클래스를 상속받을 수 없기 때문에,
