...
public class F_operator {
    ...
    // 사용법 : value >> n은 value의 비트를 n만큼 오른쪽으로 이동시킨다.
    // 효과 : 숫자를 2의 n제곱으로 나누는 것과 동일하다. (부호가 유지된다.)
    // 3. 부호 없는 오른쪽 쉬프트 연산자(>>>)
    // 동작 : 지정된 비트 수만큼 오른쪾으로 비트를 이동시킨다. 오른쪽으로 이동한 빈 자리는 0으로 채워진다.
    // 이 연산은 부호 비트를 무시하고, 양수로만 취급한다.
    // 사용법 : value >>> n은 value의 비트를 n만큼 오른쪽으로 이동시킨다.
    // 효괴 : 부호 없는 비트 이동이 필요할 때 사용된다.
    ...
    public static void exam7() {
        ...
        //2.
        value = -8; // -8은 2진수로 1111 1000 (23비트로 표현할 때)
