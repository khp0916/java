...
public class K_member_management {
    ...
    public static void addMember(String[][] members) {
        // -> 예외처리 1
        // 인원수 예외 처리를 해주세요.
        // 인원이 초과됐으면, ... "회원이 꽉 찼습니다." 를 출력하고 addMember 함수를 종료시켜주세요.
    ...
    public static boolean checkEmail(String[][] members, String email) {
        // 회원이 존재하면 true, 존재 하지 않으면 : false 를 반환한다.
    ...
    public static void selectName(String[][] members) {
        ...
        // 이름은 중복이 허용된다...
        // 다 출력해야한다.
        // ** 고민 포인트 : 1번이라도 찾았다는 것을 인지할 수 있는 어떠한 flag 값이 필요하다.
        boolean flag = false;
        for ( int i = 0; i < totalCnt; i++) {
            if (name.contains(members[i][0])) {
                System.out.println("[이름] " + members[i][0] + ", [이메일] " + members[i][1] + ", [연락처] " + members[i][2]);
                flag = true;
            }
        }
    ...
    public static void updateMember(String[][] members) {
        ...
        members[updateNum][0] = newName;
        members[updateNum][1] = newEmail;
        members[updateNum][2] = newPhone;
    }
    ...
    public static void deleteMember(String[][] members) {
        ...
        members[totalMemberCnt][0] = null;
        members[totalMemberCnt][1] = null;
        members[totalMemberCnt][2] = null;
